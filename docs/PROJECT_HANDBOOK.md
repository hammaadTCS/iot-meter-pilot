...
>
> **Two conventions, so this stays true.** Links are relative to `docs/` — code is
> `../app/…`, a sibling doc is just `NAME.md`; `DocumentationLinksTest` fails the
> build if one does not resolve. And links point at *files*, never at line
> numbers: an `#L120` anchor is a promise about every line above it too, so it
> rots on the next reformat (the Pint pass alone invalidated anchors across 73
> files). Name the method in the prose instead — it is searchable and it survives.
>
...
These are known, accepted limits — not bugs, and not things anyone forgot. Each is
paired with the condition that forces action, in the same idiom
[PENDING_WORK.md](PENDING_WORK.md) §2 uses for delivery scaling. **Read this before
sizing infrastructure or committing anything to a customer.** Full reasoning in
[THREAT_MODEL.md](THREAT_MODEL.md) §4.

| The system cannot… | Detail | Trigger that forces action |
|---|---|---|
| **Verify that a meter reports honest readings** | Broker credentials prove *which* device is talking, not that its data is true. The payload validator checks `is_numeric` and nothing else — no plausibility bounds. A device can lie about itself (not about others; topic ACLs stop that). | **Before consumption data drives money** — billing, tariffs, settlement, or any customer-facing figure with financial consequence |
| **Handle the 10,000-device target** | Ingestion is a single-threaded, file-locked, single-instance consumer doing ~10 queries per message. 10K devices at 1/min ≈ 167 msg/s ≈ 1,670 queries/s in one PHP process — not reachable. Realistic ceiling likely **1–3K devices**; needs measuring (item A19), and the fix is architectural. | Ingest lag stops returning to zero overnight |
| **Survive the ingestion process dying** | One consumer, no failover. Detection is planned; failover is not. Running a second consumer needs a new dedupe strategy first, or rollups double-count. | Any availability commitment to a customer |
| **Rely on CSP to stop XSS** | `script-src` keeps `'unsafe-inline'` (inline dashboard JS) *and* `'unsafe-eval'` (Alpine compiles directive expressions with `new AsyncFunction`). Output escaping is the primary defence, not a backstop. | Re-audit escaping whenever user data reaches a new view or broadcast payload |
| **Send any email** | `MAIL_MAILER=log`. No alert emails, no password reset, no email verification — so account email addresses are unproven. Deliberately deferred; see [PENDING_WORK.md](PENDING_WORK.md) §4a. | Before FGAC Phase 7, and before device claiming ships |
| **Tell you who changed someone's permissions** | Roles and grants are synced with no audit record. | First external user with elevated permissions, or any compliance review |
...
- **`match` expressions** (like Python's `match` but returns a value):
  ...
  ```
  Real example: [Device.php](../app/Models/Device.php).

- **Named arguments** (same as Python kwargs):
  ```php
  $this->ingestionRecorder->record(topic: $topic, status: 'unknown_topic');
  ```
  Real example: [MeterPayloadProcessor.php](../app/Services/Meters/MeterPayloadProcessor.php).
...
- **`$fillable`** — whitelist of columns allowed in mass assignment
  ...
  save, this is why. See [Device.php](../app/Models/Device.php).
...
- **Listener** = a class with a `handle()` method whose type-hint declares which
  ...
  in [AppServiceProvider.php](../app/Providers/AppServiceProvider.php).)
...
| **laravel/sanctum** | API token auth | protects `/api/*`; browser JS calls the API using its session cookie via `statefulApi()` ([bootstrap/app.php](../bootstrap/app.php)) |
...
| **Chart.js** (CDN) | charting | all charts on the meter dashboard ([meter.blade.php](../resources/views/devices/dashboards/meter.blade.php)) |
...
> load-bearing — the maintained documentation is in [docs/]().
...
already exists at [docs/erd.md](erd.md); here is the annotated version:
...
[MeterPayloadProcessor::updateMonthlyConsumption()](../app/Services/Meters/MeterPayloadProcessor.php)
...
[User.php](../app/Models/User.php). Sessions are cookie-based (config in
...
> import is commented out at [User.php](../app/Models/User.php)). Laravel's
...
Three helper methods on [User.php](../app/Models/User.php):
...
3. **Query scoping** — lists never *fetch* what you shouldn't see:
   `Device::forUser($user)` ([Device.php](../app/Models/Device.php))
...
> [docs/PERMISSIONS_HANDBOOK.md](PERMISSIONS_HANDBOOK.md)** · engineering
> plan + status ledger: [docs/FGAC_IMPLEMENTATION_PLAN.md](FGAC_IMPLEMENTATION_PLAN.md)
> · raw matrix: [docs/FGAC_FEATURES_PERMISSIONS.csv](FGAC_FEATURES_PERMISSIONS.csv).
...
(routes at [web.php](../routes/web.php)); views
...
(`.../data` → `.../status`) by
[Device::deriveAvailabilityTopic()](../app/Models/Device.php).
...
|---|---|---|---|
| Is telemetry fresh? | [healthStatus()](../app/Models/Device.php) | online / stale / down / never_seen / disabled | `last_seen_at` vs thresholds in [config/meter-health.php](../config/meter-health.php) (stale ≥ 180 s, down ≥ 600 s, env-overridable) |
| What does MQTT availability say? | [availabilityStatus()](../app/Models/Device.php) | online / offline / silent / unknown / disabled | `last_availability_*` columns; "silent" = broker says online but no telemetry; an explicit *offline* is authoritative until newer telemetry/heartbeat supersedes it |
| Is the latest payload broken? | [issueStatus()](../app/Models/Device.php) | ok / error / recovered / disabled | `last_error_*` vs `last_recovered_at` |
...
1. Takes an **exclusive file lock** (`storage/framework/mqtt-consumer.lock`,
   [`acquireConsumerLock()`](../app/Console/Commands/ConsumeMeterTopic.php)) so two
   consumers can never run at once (double-processing guard).
2. Installs **SIGTERM/SIGINT handlers** ([`installSignalHandlers()`](../app/Console/Commands/ConsumeMeterTopic.php))
...
5. If stored **and recent** (< 120 s old, [the `$isLiveReading` catch-up check](../app/Console/Commands/ConsumeMeterTopic.php)),
...
6. On broker disconnect: reconnects with **exponential backoff + jitter**
   ([`nextRetryDelaySeconds()`](../app/Console/Commands/ConsumeMeterTopic.php)). After 50 000
...
4. **In one DB transaction** ([the ingestion transaction](../app/Services/Meters/MeterPayloadProcessor.php)):
   ...
   - Decide **promotion**: history accepts out-of-order messages (normal in
     ...
     ([shouldPromoteToLatestState()](../app/Services/Meters/MeterPayloadProcessor.php)).
...
[its `fetch()` calls](../resources/views/devices/dashboards/meter.blade.php)).
...
- **The notification bell** — the real-time path in production use. When an
  ...
  ([header.blade.php](../resources/views/components/header.blade.php))
...
> catalogued in [docs/ALERT_CATALOG.md](ALERT_CATALOG.md).** This section
...
- [ScanMeterHealth](../app/Console/Commands/ScanMeterHealth.php) — telemetry
  ...
  *availability* topic is designed but **not yet built** — see
  [docs/ALERT_CATALOG.md](ALERT_CATALOG.md) §3.)
...
([DeviceReadingController.php](../app/Http/Controllers/DeviceReadingController.php)).
Note the routing subtlety documented at [api.php](../routes/api.php):
...
[docs/DEPLOY_PARTS_1_2_CHECKLIST.md](DEPLOY_PARTS_1_2_CHECKLIST.md).
...
| **FGAC** | fine-grained access control — the planned permission system replacing role checks ([docs/FGAC_IMPLEMENTATION_PLAN.md](FGAC_IMPLEMENTATION_PLAN.md)) |
